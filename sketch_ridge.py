...
track_primal = np.zeros((20, 3))
rep = 10
lbd_seq = [0.1, 0.3, 0.5]
theory_primal = np.zeros((100, 3))
for j in range(1):
    lbd = lbd_seq[j]
    for i in range(20):
        xi = c[i]
        ...
        for k in range(rep):
            ...
            beta_primal = np.linalg.inv(X.T @ L.T @ L @ X / n + lbd * np.identity(p)) @ X.T @ Y / n
            track_primal[i, j] = track_primal[i, j] + np.linalg.norm(beta_primal - beta) ** 2
...
for j in range(3):
    ...
    for i in range(100):
        xi = d[i]
        theory_primal[i, j] = MSE_primal(gamma, xi, lbd, alpha, sigma)
        # the_1 = \
        #     quad(MP_moment, (1 - sqrt(gamma / xi)) ** 2, (1 + sqrt(gamma / xi)) ** 2, args=(1, gamma / xi, lbd, xi))[0]
        # the_2 = \
        #     quad(MP_moment, (1 - sqrt(gamma / xi)) ** 2, (1 + sqrt(gamma / xi)) ** 2, args=(2, gamma / xi, lbd, xi))[0]
        # # the_1 = theta_1(gamma, lbd)
        # # the_2 = theta_2(gamma, lbd)
        # bias = alpha ** 2 * ((lbd + xi - 1) ** 2 + gamma * (1 - xi)) * the_2
        # variance = gamma * sigma ** 2 * (the_1 - (lbd + xi - 1) * the_2)
        # theory_primal[i, j] = bias + variance

for j in range(1):
    lbd = lbd_seq[j]
    plt.scatter(c[:], track_primal[:, j] / rep, label='Simulation, $\lambda$={}'.format(lbd))
    plt.plot(d[:], theory_primal[:, j], label='Theory, $\lambda=${}'.format(lbd))

plt.xlabel(r'$\xi$', fontsize=14)
plt.ylabel(r'$MSE(\hat\beta)$', fontsize=14)
plt.grid(linestyle='dotted')
plt.title('MSE primal sketch')
plt.legend(fontsize=10)
plt.savefig('primal_lambda=0.3,0.5.png')
...
alpha = 1
sigma = 1
c = np.linspace(0.1, 0.5, 20)
track_dual = np.zeros((20, 3))
rep = 10
lbd_seq = [0.1, 0.3, 0.5]
theory_dual = np.zeros((100, 3))
for j in range(1):
    lbd = lbd_seq[j]
    for i in range(20):
        zeta = c[i]
        ...
        for k in range(rep):
            ...
            track_dual[i, j] = track_dual[i, j] + np.linalg.norm(beta_dual - beta) ** 2

zeta_seq = np.linspace(0.1, 0.5, 100)
for j in range(3):
    lbd = lbd_seq[j]
    for i in range(100):
        zeta = zeta_seq[i]
        theory_dual[i, j] = MSE_dual(gamma, zeta, lbd, alpha, sigma)
        # the_1 = quad(MP_moment, (1 - sqrt(zeta)) ** 2, (1 + sqrt(zeta)) ** 2, args=(1, zeta, lbd, 1))[0]
        # the_2 = quad(MP_moment, (1 - sqrt(zeta)) ** 2, (1 + sqrt(zeta)) ** 2, args=(2, zeta, lbd, 1))[0]
        # barthe_1 = (1 - zeta) / lbd + zeta * the_1
        # barthe_2 = (1 - zeta) / lbd ** 2 + zeta * the_2
        # # print(barthe_1, barthe_2)
        # bias_1 = alpha ** 2 / gamma * (
        #         1 + (lbd + zeta - gamma) ** 2 * barthe_2 + 2 * (gamma - zeta - lbd) * barthe_1 + (
        #         gamma - zeta) * barthe_1 ** 2)
        # bias_2 = -2 * alpha ** 2 / gamma * (1 - (lbd + zeta - gamma) * barthe_1)
        # bias = bias_1 + bias_2 + alpha ** 2
        # print(bias - alpha ** 2 / gamma * (
        #         gamma - 1 + (lbd + zeta - gamma) ** 2 * barthe_2 + (gamma - zeta) * barthe_1 ** 2))
        # variance = sigma ** 2 * (barthe_1 - (lbd + zeta - gamma) * barthe_2)
        # # print(bias_1, bias_2, bias, variance)
        # theory_dual[i, j] = bias + variance

for j in range(1):
    lbd = lbd_seq[j]
    plt.scatter(c, track_dual[:, j] / rep, label='Simulation $\lambda$={}'.format(lbd))
    plt.plot(zeta_seq, theory_dual[:, j], label='Theory $\lambda$={}'.format(lbd))

plt.xlabel(r'$\xi$', fontsize=14)
plt.ylabel(r'$MSE(\hat\beta)$', fontsize=14)
plt.grid(linestyle='dotted')
plt.title('MSE dual sketch')
plt.legend(fontsize=10)
plt.savefig('dual_lambda=0.3,0.5.png')
