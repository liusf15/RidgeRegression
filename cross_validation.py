...
batch_size = int(n / K)
steps = 40
lbd_seq = np.linspace(0.001, 1, steps)
...
for j in range(steps):
    lbd = lbd_seq[j]
    beta_ridge = np.linalg.inv(X_train.T @ X_train / n + lbd * np.identity(p)) @ X_train.T @ Y_train / n
...
plt.errorbar(lbd_seq, np.mean(CV_err, 1), np.sqrt(np.var(CV_err, 1)), capsize=2, label='CV errorbar')
plt.plot(lbd_seq, test_err, label='Test error', linewidth=2)
plt.plot(one_std_err_lbd * np.ones(10), np.linspace(1.25, 1.65, 10), ls=':', color='red', linewidth=2, label='One-Std')
plt.plot(lbd_seq, np.mean(CV_err, 1)[one_std_err_idx] * np.ones(steps), ls=':', color='red', linewidth=2)
plt.plot(lbd_cv * np.ones(10), np.linspace(1.25, 1.65, 10), ls='--', linewidth=3, label='CV min')
plt.plot(lbd_cv_debiased * np.ones(10), np.linspace(1.25, 1.65, 10), ls='-.', linewidth=3, label='Debiased CV min')
plt.plot(lbd_smallest * np.ones(10), np.linspace(1.25, 1.65, 10), ls='-.', label='Test error min', linewidth=3)
plt.grid(linestyle='dotted')
plt.xlabel(r'$\lambda$', fontsize=14)
plt.ylabel('CV test error')
plt.title('CV test error')
plt.legend(fontsize=12)
plt.savefig('CV_test_error.png')
